...
class Resolver {
  ...
  private resolveStmt(statement: Declaration): void {
    return match(statement)
      .with({ stmtType: P.union("PRINT", "EXPR") }, ({ expr }) => this.resolveExpr(expr))
      .with({ stmtType: "RETURN" }, ({ expr, token }) => {
          if (this.currentFunctionType === FunctionType.NONE)
            this.resolverErrorCallback(token, "Can't return from top-level code.")
      ...
      .with(
        ...
        ({ funName, params, funBody }) => {
          ...
          this.currentFunctionType = FunctionType.FUNCTION
  ...
  private declareVar(identifier: Identifier) {
    ...
    if (scope.has(identifier.lexeme))
      this.resolverErrorCallback(identifier, "Already a variable with this name in this scope.")
